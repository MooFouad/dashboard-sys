...
router.get('/count/total', async (req, res, next) => {
  try {
    ...
    if (process.env.USE_MOCK_DATA === 'true') {
      count = await mockDataService.count('absher');
    } else {
      count = await Absher.countDocuments();
    }
    res.json({
      success: true,
      count
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (process.env.USE_MOCK_DATA === 'true') {
      let records = await mockDataService.find('absher');
      ...
      if (search) {
        ...
        records = records.filter(r =>
          ...
          (r.ownerId && r.ownerId.includes(search))
        );
      }
