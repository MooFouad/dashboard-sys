...
router.get('/count/total', async (req, res, next) => {
  try {
    ...
    if (process.env.USE_MOCK_DATA === 'true') {
      count = await mockDataService.count('socialInsurance');
    } else {
      count = await SocialInsurance.countDocuments();
    }
    res.json({
      success: true,
      count
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (process.env.USE_MOCK_DATA === 'true') {
      let records = await mockDataService.find('socialInsurance');
      ...
      if (search) {
        ...
        records = records.filter(r =>
          ...
          (r.nationalId && r.nationalId.includes(search)) ||
          (r.division && r.division.toLowerCase().includes(searchLower))
        );
      }
      ...
      // Sort by end date (soonest to expire first)
      ...
      const paginatedRecords = formattedRecords.slice(startIndex, startIndex + parseInt(limit));
