...
const gosiMockService = require('./mocks/gosiMockService');
...
class GOSIService {
  ...
  async getAccessToken() {
    ...
    if (process.env.DEMO_MODE === 'true') {
      return gosiMockService.getAccessToken();
    }
  ...
  async getContributorData(ninOrIqama, retryCount = 0) {
    // Use mock service in demo mode
    if (process.env.DEMO_MODE === 'true') {
      const mockData = await gosiMockService.fetchEmployeeData(ninOrIqama);
      return {
        success: true,
        data: mockData
      };
